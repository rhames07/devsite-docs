...
> IMPORTANTE
>
> * Debes realizar los pasos 2 y 3 para obtener el ACCESS_TOKEN de tu nuevo usuario de pruebas.
...
Es necesario que registres tu aplicación en nuestra API mediante tu **Access Token** para comenzar a utilizarla, con este servicio podrás darte de alta:

``` cURL
curl --location --request POST 'https://api.mercadopago.com/point/integration-api/integrators' \
...
``` json
{
    "token": "45d914ed-104e-49ec-af74-2bd293e2f8b8"
}
```
Guarda el **token** en un lugar seguro, este será requerido al momento de configurar tu webhook.
...
- **Validación de seguridad HTTP GET:** Esta operación es requerida para validar que el sistema webhook configurado te pertenece.
  ...
  El algoritmo requiere de un `secret` que corresponde al `token` obtenido en el **Paso 2** y de un message que corresponde al `challengeCode`.
...
### Posibles estados finales de una intención de pago
...
## 4. Asocia tu dispositivo Point a tu cuenta de Mercado Pago
> NOTE
>
...
#### 4.1. Escanea el código QR de tu dispositivo Point
...
#### 4.2. Configura tu tienda y tu caja
...
#### 4.3. Activa el modo integrado en tu dispositivo Point
------------------------***PENDIENTE***-------------------
